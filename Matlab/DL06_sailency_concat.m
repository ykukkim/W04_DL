    ...
    for j = 1:size(TestPath1_type,1)
        ...
        for k = 1:size(TestPath1_markers,1)
            ...
            for t = 1:size(TestPath1_total,1)
                ...
                xlsxList = dir([TestPath1_Final(k).folder,pathCompSep,'saliency',pathCompSep,'*.csv']);
                for v = 1:size(xlsxList,1)
                    Nr = [];
                    [FSnum,FSstr]            = xlsread([xlsxList(v).folder,pathCompSep,xlsxList(v).name],1);
                    sum_FS = sum(FSnum(:,2:end));
                    Nr = sum_FS;
                    table_path = fullfile([TestPath1_Final(k).folder,pathCompSep,'saliency',pathCompSep,(names{v}),'-','saliency_Total.csv']);
                    writetable(array2table(Nr, 'VariableNames', FSstr), table_path)
